...
```{r}
print (new_names<-ifelse(nchar(murders$state)>8, murders$abb, murders$state))
```
...
7. Write a function compute_s_n that for any given n computes the sum Sn=12+22+32+…n2. Report the value of the sum when n=10.
```{r}
nplustwo <- function(x){x*10+2}
compute_s_n <- function(n){ 
  x <- 1:n
  sum(sapply(x, nplustwo)) }
compute_s_n(10)
```

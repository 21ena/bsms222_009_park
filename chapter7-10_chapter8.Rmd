...
murders %>% ggplot(aes(population, total)) +
  geom_label(data=murders,
             mapping=aes(label=state),color ="blue")
```
...
murders %>% ggplot(aes(population, total)) +
  geom_label(data=murders,
             mapping=aes(label=state, col=region))
```
...
Define a vector of quantiles q1,…,qm for your data for the proportions  p1,…,pm. We refer to these as the sample quantiles.
Define a vector of theoretical quantiles for the proportions p1,…,pm for a normal distribution with the same average and standard deviation as the data.
...
Percentiles are special cases of quantiles that are commonly used. The percentiles are the quantiles you obtain when setting the p at  0.01,0.02,...,0.99. We call, for example, the case of  p=0.25
...
```
```{r}
pnorm(72, mean=m, sd=s, lower.tail = TRUE)-pnorm(69, mean=m, sd=s, lower.tail = TRUE)
```
...
s <- sd(x)
c(average = m, sd = s)
pnorm(81, mean=m, sd=s, lower.tail = TRUE)-pnorm(79, mean=m, sd=s, lower.tail = TRUE)
```
5/3배 가량 차이남
...
```{r}
1-pnorm(84, m=69, sd=3, lower.tail = TRUE)
```
...
```{r}
1-pnorm(80, m=69, sd=3, lower.tail = TRUE)
```
```{r}
0.0001228664*1000000000
```
```{r}
150/122866.4*100
```
